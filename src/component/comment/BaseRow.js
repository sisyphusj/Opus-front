...
const BaseRow = ({ data }) => {
    ...
    const [, setPinOpen] = useRecoilState(pinModalOpenState);
    ...
    // open 상태가 변경될 때 핀 데이터를 가져오는 로직
    useEffect(() => {
        if (open && isLogin) {
            getPinByPId(data.pinId);
        }
    }, [open, isLogin, data.pinId]);
                        ...
                        <Box sx={{ margin: 1 }}>
                            <Typography variant="h6" gutterBottom component="div">
                                <Flex justifyContent={"between"}>
                                    <Flex>
                                        {open && currentPin && (
                                            <Avatar src={currentPin.imagePath} alt={String(data.pinId)}
                                                    sx={{
